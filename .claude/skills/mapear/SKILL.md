---
name: mapear
description: Use para mapear um codebase existente (brownfield) e produzir docs/arquitetura/assessment.md — stack, arquitetura, bounded contexts implícitos, maturidade nos 5 eixos, dívidas/riscos e decisões históricas a virar ADR retroativo. Re-execução atualiza o assessment. É chamada pelo /kickoff no modo brownfield, e também roda sozinha quando o codebase mudar ou para analisar um repo. Acione com /mapear.
---

# Skill: Mapear o estado atual (as-is)

Produz o retrato de um projeto **já em andamento**. Primeiro **leia o código**, depois pergunte
só o que o código não revela. É **idempotente**: re-rodar atualiza `docs/arquitetura/assessment.md`.
...
1. **Mapeamento automático:** identifique stack, estrutura de pastas, estilo de arquitetura,
   ...
   exploração (ver `docs/arquitetura/engenharia/_templates/subagent.template.md`) para manter o contexto enxuto.
...
## Saídas
- `docs/arquitetura/assessment.md` (use `docs/arquitetura/_templates/assessment.template.md`).
- Lista de ADRs retroativos a criar em `docs/arquitetura/adr/`.

---
name: handoff
description: Use ao PAUSAR/encerrar uma sessão de trabalho (grava o estado atual em docs/operacao/estado.md para retomar depois) ou ao RETOMAR (lê docs/operacao/estado.md e a spec ativa e recompõe o contexto, propondo o próximo passo). Mantém a continuidade entre sessões de humanos e agentes. Acione com /handoff.
---

# Skill: Handoff de sessão (pause / resume)

Mantém a continuidade do projeto via `docs/operacao/estado.md` — a memória de trabalho volátil
(diferente do ADR, que é decisão durável). Detecte a intenção pelo pedido ou pergunte.

## Modo PAUSE (pausar / encerrar)
Atualize `docs/operacao/estado.md` com o estado real desta sessão:
...
5. **Consolide `docs/operacao/aprendizados.md`** (condicional — só se houve aprendizado): a captura das lições é
...
## Modo RESUME (retomar)
1. Leia `docs/operacao/estado.md` e a `spec.md`/`tasks.md` da feature ativa citada nele.

...
`docs/operacao/estado.md` · `docs/produto/visao.md` · `docs/produto/roadmap.md` · e a `spec.md` da
...
## Linguagem ubíqua
- Use **exatamente** os termos de `docs/glossario.md` e do `domain.md` da feature.
...
**Base (`alwaysApply: true`):** este `CLAUDE.md` · `docs/operacao/estado.md` · `docs/produto/visao.md` ·
`docs/produto/roadmap.md` · a `spec.md` da feature ativa. Todo o resto é sob demanda — puxe
...
- **Delegue para manter o contexto enxuto:** pesquisa e tasks paralelas (`[P]`) vão para
  subagentes (ver `docs/arquitetura/engenharia/_templates/subagent.template.md`), que recebem só a task +
...
- [ ] Decisões difíceis de reverter viraram ADR em `docs/arquitetura/adr/`
- [ ] Glossário e `docs/arquitetura/mapa-de-contexto.md` atualizados se mudaram
- [ ] A spec reflete o que foi construído (ou a divergência está documentada)
- [ ] `docs/operacao/estado.md` atualizado (próximo passo, decisões, bloqueios)

## Memória de trabalho — `docs/operacao/estado.md`
...
## Aprendizados & gotchas — `docs/operacao/aprendizados.md`
- Ao **corrigir um erro não-óbvio, quebrar uma suposição ou descobrir um gotcha** (quirk de lib,
  armadilha de setup, rate-limit), **registre 1 linha em `docs/operacao/aprendizados.md` na hora** — não espere
...
## Onde escrever
- Decisão arquitetural durável → novo ADR (`docs/arquitetura/adr/`), nunca edite ADR antigo (são imutáveis; crie um que o substitua).
- Estado do trabalho / próximo passo → `docs/operacao/estado.md`.
- Aprendizado transversal / gotcha (ainda não estabilizado) → `docs/operacao/aprendizados.md`.
- Termo de negócio → `docs/glossario.md`.
- Mudança de fronteira/contexto → `docs/arquitetura/mapa-de-contexto.md`.
